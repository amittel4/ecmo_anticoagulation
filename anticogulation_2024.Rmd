...
# adding nearest platelet count within 24 hours of aPTT (mirrors nearest_bilirubin)
platelet_labs = 
  run_labs_complications_raw %>% unnest(data) %>% 
  filter(!is.na(platelet)) %>%
  select(study_id, mrn, lab_result_time, platelet)

coag_platelet_matches = fuzzyjoin::fuzzy_left_join(
  coag_labs,
  platelet_labs,
  by = c("study_id" = "study_id", "lab_result_time" = "lab_result_time"),
  match_fun = list(`==`, function(x, y) abs(difftime(x, y, units = "hours")) <= 24)
...
covar_snapshot <- model_fit_complications %>%
  ...
  ungroup()
  # If you want to restrict to labs within 24 hours of index_time:
