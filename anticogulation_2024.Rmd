...
```{r survival_index_covariates, echo = FALSE, warning = FALSE}
###############################################################################
# 1. Index time per patient: first discordance or ECMO start
###############################################################################
index_times <- surv_base %>%
  mutate(
    index_time = dplyr::if_else(
      !is.na(first_discord_time),
      first_discord_time,
      time_on
    )
  ) %>%
  select(mrn, index_time)

###############################################################################
# 2. Nearest covariate snapshot at index time (from model_fit_complications)
###############################################################################
covar_snapshot <- model_fit_complications %>%
  select(
    mrn,
    lab_result_time,
    nearest_inr,
  ...
  mutate(
    time_diff_hours = abs(as.numeric(difftime(lab_result_time, index_time, units = "hours")))
  ) %>%
  group_by(mrn) %>%
...
covar_snapshot %>%
  summarise(
    n_patients_with_covars = n_distinct(mrn),
    median_time_diff_hours = median(time_diff_hours, na.rm = TRUE),
    max_time_diff_hours    = max(time_diff_hours, na.rm = TRUE)
  ) %>%
  knitr::kable(digits = 2,
               caption = "Time difference (hours) between index time and matched lab per patient")
...
thromb_indexed_results %>%
  knitr::kable(
    digits = 3,
    caption = "Time-varying Cox model (thrombosis): discordance + labs at time of first discordance"
  )
...
bleed_indexed_results %>%
  knitr::kable(
    digits = 3,
    caption = "Time-varying Cox model (bleeding): discordance + labs at time of first discordance"
  )
```
